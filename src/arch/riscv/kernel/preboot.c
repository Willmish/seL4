...
// TODO(aappleby): Replace with autogenerated addresses once available.
volatile uint32_t* inbox_queue = (volatile uint32_t*)0x400F1010;
...
BOOT_CODE void preinit_kernel(void)
{
  ...
  paddr_t ui_p_reg_start = (paddr_t)*inbox_queue; // user app paddr
  paddr_t ui_p_reg_end = (paddr_t)*inbox_queue;   // user app paddr
  sword_t pv_offset = (sword_t)*inbox_queue;      // user app virt-to-phys offset
  vptr_t v_entry = (vptr_t)*inbox_queue;          // user app vaddr entry point
